...
cv2.createTrackbar('contrast', 'Filter', 1, 100, dummy)
cv2.createTrackbar('brightness', 'Filter', 50, 100, dummy)
cv2.createTrackbar('filter', 'Filter', 0, len(kernels)-1, dummy)
cv2.createTrackbar('grayscale', 'Filter', 0, 1, dummy)

count = 1

while True:
	grayscale = cv2.getTrackbarPos('grayscale', 'Filter')
	if grayscale == 0:
		cv2.imshow('Filter', color_modified)
	else:
		cv2.imshow('Filter', gray_modified)
	...
	contrast = cv2.getTrackbarPos('contrast', 'Filter')
	brightness = cv2.getTrackbarPos('brightness', 'Filter')
	kernel = cv2.getTrackbarPos('filter', 'Filter')
